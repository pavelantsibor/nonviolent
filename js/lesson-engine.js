...
import { isLastStepInSection } from "./content/catalog.js";
...
export class LessonEngine {
  ...
  _buildOfnrSummary() {
    const max = { observation: 0, feeling: 0, need: 0, request: 0 };
    for (const s of this.steps) {
      const f = inferOfnrFocus(s);
      if (f === "mixed" || max[f] === undefined) continue;
      if (s.type === "dialog") {
        const possible = Object.values(s.nodes || {}).reduce(
          (acc, node) => acc + (node.choices?.length || 0) * 2,
          0
        );
        max[f] += possible;
      } else {
    ...
    for (const k of Object.keys(max)) {
      percent[k] = max[k] ? Math.min(100, Math.round((this._ofnrPoints[k] / max[k]) * 100)) : 0;
    }
    return {
      earned: { ...this._ofnrPoints },
      max,
      percent,
      labels: OFNR_LABELS,
    };
