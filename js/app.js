...
/** Визуализация 1–3 звёзд (оценка раздела или модуля) */
function renderStarsHtml(n) {
  const stars = Math.min(3, Math.max(1, Math.round(Number(n)) || 1));
  const filled = "★".repeat(stars);
  const empty = "☆".repeat(3 - stars);
  const label = `${stars} из 3`;
  return `<span class="star-rating" aria-label="${escapeHtml(label)}" title="${escapeHtml(label)}"><span class="star-rating__on">${filled}</span><span class="star-rating__off" aria-hidden="true">${empty}</span></span>`;
}
...
function showHome() {
  ...
  const cards = modules
    .map((m) => {
      ...
      const modStars = storage.getModuleStars(m.id);
      const starsRow =
        typeof modStars === "number"
          ? `<p class="card__meta card__meta--stars">${renderStarsHtml(modStars)} <span class="muted">оценка модуля</span></p>`
          : "";
      return `
...
function showModuleMap(moduleId) {
  ...
  const sectionMaxMap = buildSectionMaxMap(mod);
  const secStars = storage.getModuleSectionStars(moduleId);
  const moduleStarVal = storage.getModuleStars(moduleId);
  if (slices.length === 0) {
    withViewTransition(() => {
      app.innerHTML = `
  ...
  const pathItems = slices.map((slice, i) => {
    ...
    const startBadge =
      status === "current"
        ...
        : "";
    const smax = sectionMaxMap[slice.id] || 0;
    const sVal = secStars[slice.id];
    const starLine =
      smax > 0
        ? typeof sVal === "number"
          ? `<span class="module-path__stars">${renderStarsHtml(sVal)}</span>`
          : `<span class="module-path__stars module-path__stars--empty muted">оценка после прохождения</span>`
        : "";
    return `
    ...
    <section class="screen is-active module-map-screen">
      <div class="module-map-header">
        ...
        <div class="module-map-header__text">
          ...
          ${
            typeof moduleStarVal === "number"
              ? `<p class="module-map-module-stars">${renderStarsHtml(moduleStarVal)} <span class="muted">оценка за весь модуль</span></p>`
              : ""
          }
        </div>
      </div>
...
function startLesson(moduleId, fromStepIndex) {
  ...
  if (typeof fromStepIndex === "number" && !Number.isNaN(fromStepIndex)) {
    ...
    if (jumpOrReplay) {
      ...
      storage.clearModuleSectionEarnedPartial(moduleId);
    }
  }
  ...
  const initialSectionEarned = storage.getModuleSectionEarnedPartial(moduleId);
  ...
  currentEngine = new LessonEngine(mod, container, {
    ...
    onSectionComplete: (m, meta) => {
      ...
      if (meta.sectionMax > 0) {
        storage.setModuleSectionStar(m.id, meta.sectionId, meta.sectionStars);
      }
      if (meta.sectionEarnedMap && typeof meta.sectionEarnedMap === "object") {
        storage.setModuleSectionEarnedPartial(m.id, meta.sectionEarnedMap);
      }
      currentEngine = null;
      showModuleMap(m.id);
    },
    onComplete: (m, meta) => {
      ...
      storage.clearModuleSectionEarnedPartial(m.id);
      storage.setStepIndex(m.id, 0);
      const summ = meta?.stars;
      if (summ) {
        storage.setModuleStars(m.id, summ.moduleStars);
        for (const sec of summ.sections) {
          if (sec.max > 0) storage.setModuleSectionStar(m.id, sec.id, sec.stars);
        }
      }
      currentEngine = null;
      const points = meta?.points ?? 0;
      if (isFirstGift && m.badge) {
        showGiftCelebrationModal(m, () => showSummary(m, points, summ));
      } else {
        showSummary(m, points, summ);
      }
    },
  });
  ...
  const onLessonExit = () => {
    ...
    if (eng) {
      ...
      if (snap.sectionEarned && typeof snap.sectionEarned === "object") {
        storage.setModuleSectionEarnedPartial(moduleId, snap.sectionEarned);
      }
    }
    currentEngine = null;
...
function showSummary(mod, sessionPoints, starSummary) {
  const gift = mod.badge?.giftTitle || mod.badge?.label || "подарок";
  const giftDesc = mod.badge?.giftDescription || "";
  const modStars = starSummary?.moduleStars;
  const scoredSections = (starSummary?.sections || []).filter((s) => s.max > 0);
  const sectionRows = scoredSections
    .map((s) => {
      const weak = s.stars < 3;
      return `<li class="star-summary__item${weak ? " star-summary__item--weak" : ""}">
        <span class="star-summary__name">${escapeHtml(s.title)}</span>
        <span class="star-summary__stars">${renderStarsHtml(s.stars)}</span>
        <span class="muted star-summary__pts">${s.earned}/${s.max}</span>
      </li>`;
    })
    .join("");
  const starsBlock =
    scoredSections.length > 0
      ? `<div class="card star-summary">
        <h3 style="margin:0 0 6px;font-size:1rem">Оценка по разделам</h3>
        <p class="muted" style="margin:0 0 10px;font-size:0.85rem">★★★ — сильно, ★★ — можно лучше, ★ — повторите раздел на карте модуля.</p>
        ${
          typeof modStars === "number"
            ? `<p class="star-summary__module-line">Весь модуль: ${renderStarsHtml(modStars)}</p>`
            : ""
        }
        <ul class="star-summary__list">${sectionRows}</ul>
      </div>`
      : "";
  withViewTransition(() => {
  app.innerHTML = `
